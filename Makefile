PREFIX          ?= $(HOME)/.local
BINDIR           = $(PREFIX)/bin
EXTENSION_UUID   = ff-profiles@seblebs.de
...
install-desktop:
	install -Dm644 data/de.seblebs.ff-profiles.desktop \
		$(DESKTOP_DIR)/de.seblebs.ff-profiles.desktop

uninstall:
	rm -f  $(BINDIR)/ff-profiles
	rm -rf $(EXTENSION_DIR)
	rm -f  $(DESKTOP_DIR)/de.seblebs.ff-profiles.desktop
